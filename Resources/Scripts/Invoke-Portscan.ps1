...
"@
            ...
            while (($saveIteration * $SyncFreq) -lt $hostList.Count)
            {
                ...
                foreach ($iHost in $hostList[$sIndex..$eIndex])
                {
                    ...
                    if(!$noProgressMeter)
                    {
                        Write-Progress -Activity $startMsg -CurrentOperation "Starting Computer $computersDone"  -PercentComplete ($computersDone / $hostList.Count * 100)
                    }
